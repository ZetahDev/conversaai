...
export const onRequest = defineMiddleware(async (context, next) => {
  ...
  if (config.security.enableRateLimiting) {
    ...
    const isSensitive = sensitiveEndpoints.some(endpoint => pathname.includes(endpoint));
    ...
    if (isSensitive) {
      ...
      if (!rateLimit.allowed) {
        ...
        response.headers.set('X-RateLimit-Limit', config.rateLimiting.loginAttempts.toString());
  ...
  if (config.isProd && config.security.enableCSRF && ['POST', 'PUT', 'DELETE', 'PATCH'].includes(method)) {
  ...
  if (config.security.enableCSRF && method === 'GET' && 
